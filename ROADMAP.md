...
### Critical Bugs
- [x] **BEMF Disabled on RP2040**: The `USE_IRQ_TRIGGER` macro is undefined in the build configuration, causing the HAL to skip interrupt registration and effectively disabling BEMF measurement.
...
### Documentation
- [x] **Missing Process Documentation**: `TESTING_IMPROVEMENTS.md` is referenced in `AGENTS.md` but does not exist.
- [ ] **Outdated Roadmap**: The roadmap previously listed deleted platforms (ESP32, nRF52, AVR) as implemented. (Corrected in this update).

### Code Quality
- [x] **Magic Numbers**: Replace hardcoded values in HAL implementations with named constants or configurable parameters.
